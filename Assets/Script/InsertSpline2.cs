...
public class InsertSplinePath3 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        OneSquareLength=GameObject.Find("StairwaySimple").transform.Find("Plane").transform.GetComponent<MeshCollider>().bounds.size.z;
        for (int i = 0; i < 4; i++)
        {
            planeWidth[i] = i*OneSquareLength / 4;
        }

        thirdProcess = OneSquareLength / 4;
        
        //ConsumeExpand = GameObject.Find("BasicUI").transform.GetComponent<HandwritingManager>();

        startPoint = new Vector3(transform.position.x + GameObject.Find("StairwaySimple").transform.Find("Plane").transform.GetComponent<MeshCollider>().bounds.size.x/2, 0, 0);
        endPoint =new Vector3(transform.position.x - GameObject.Find("StairwaySimple").transform.Find("Plane").transform.GetComponent<MeshCollider>().bounds.size.x/2, 2, 0);
        //endPoint =new Vector3(transform.position.x - GameObject.Find("StairwaySimple").transform.Find("Plane").transform.GetComponent<MeshCollider>().bounds.size.x*2, 2, 0);
    ...
    void ExtendSplineWithKnots()
    {
        ...
        for (int m = 0; m < 3; m++)
        {
            ...
            Vector3 AltStartPoint = new Vector3(-startPoint.x, 0.075f, (startPoint.z) - planeWidth[m]+thirdProcess);
            Vector3 AltEndPoint=new Vector3(-endPoint.x +  justifyWidth + 1, 0.075f, (endPoint.z)- planeWidth[m]+thirdProcess);
            
            
            // 折れ始める knot index
            
            int breakIndex = Mathf.Clamp(bendStartIndex, 1, numberOfKnots - 2);
            
            // 元の直線上の折れ点
            float breakT = breakIndex / (float)(numberOfKnots - 1);
            Vector3 breakPoint = Vector3.Lerp(AltStartPoint, AltEndPoint, breakT);

            // 元の進行方向
            Vector3 baseDir = (AltEndPoint - AltStartPoint).normalized;

            // X-Y平面で下向きに45度曲げるので、Z軸まわり
            Vector3 bentDir = (Quaternion.AngleAxis(-bendAngle, Vector3.forward) * baseDir).normalized;

            // 後半の長さを維持
            float totalLength = Vector3.Distance(AltStartPoint, AltEndPoint);
            float secondLength = totalLength * (1f - breakT);
            Vector3 bentEndPoint = breakPoint + bentDir * secondLength;
            ...
            for (int i = 0; i < numberOfKnots; i++)
            {
                ...
                {
                    // 前半: 元の直線
                    float localT = i / (float)breakIndex;
                    pos = Vector3.Lerp(AltStartPoint, breakPoint, localT);
                }
                else if(i<8)
                {
                    // 後半: 45度下向きの直線
                    float localT = (i - breakIndex) / (float)((numberOfKnots - 1) - breakIndex);
                    pos = Vector3.Lerp(breakPoint, bentEndPoint, localT);
                }
                ...
                if (numberOfKnots - 1 == i)
                {
                    ...
                    var direction = (newSpline[1].Position - newSpline[0].Position);
            ...
            float firstLength = Vector3.Distance(AltStartPoint, breakPoint);
            float lastLength = Vector3.Distance(breakPoint, bentEndPoint);
            LinapLine[m] = firstLength + lastLength;
