...
public class FirstStepInsertSplinePath2 : MonoBehaviour
{
    ...
    public int numberOfKnots = 16;
    public int SecondOfKnots = 8;
    ...
    public float HalfStep = 0;
    public float DuringHalfStep = 0;
    ...
    private Vector3 current = new Vector3(1f, 1f, 0f);
    ...
    private void Start()
    {
        ...
        //InitializeFields();
        LaneParameter lane; 
        var ans=ReverseMatrix();
        ans=ReverseMatrix();
        ans=ReverseMatrix();
        lane = new LaneParameter();
        
        ResolveSceneReferences(lane);
    ...
    private void ResolveSceneReferences(LaneParameter lane)
    {
    ...
    private void ExtendSplineWithKnots()
    {
        ...
        for (int m = 0; m < 3; m++)
        {
            ...
            LaneParameter lane = BuildLaneParameter(m, altStartPoint, altEndPoint,true);

            VerticalChanger = false;

            Vector3[] bentStack=BuildFrontHalf(newSpline, lane,true);
            Vector3 tailEnd = BuildBackHalf(newSpline,m,false,bendStartIndex-2,bendStartIndex-1,bentStack);
           if(!jurge)
            ResolveSceneReferences(lane);
            Debug.Log("");
            //BuildLaneParameter(m, altStartPoint, altEndPoint,true);
            /*altStartPoint = GetLaneStartPoint(m);
            altEndPoint = GetLaneBaseEndPoint(m);
            BuildFrontHalf(newSpline, lane,true);*/
           //BuildBackHalf(newSpline,m,false,((Vector3)newSpline[numberOfKnots - 2].Position - (Vector3)newSpline[numberOfKnots - 1].Position).normalized);
           /*BuildFrontHalf(newSpline, lane,true);

           BuildBackHalf(newSpline,false);*/
    ...
    private LaneParameter BuildLaneParameter(int laneIndex, Vector3 altStartPoint, Vector3 altEndPoint,bool CaseOfSrength)
    {
        ...
        if (laneIndex == 0)      targetX = centerX - step/2; // 右
        else if (laneIndex == 1) targetX = centerX;        // 中央
        else                     targetX = centerX+step/2; // 左
        ...
        Instantiate(GameObject.CreatePrimitive(PrimitiveType.Sphere),lane.bentEndPoint ,Quaternion.identity);

        if (CaseOfSrength)
        {
            if (laneIndex == 0)
                targetX = centerX - step; // 右
            else if (laneIndex == 1)
                targetX = centerX; // 中央
            else
                targetX = centerX + step;
        }
        else
        {
            if (laneIndex == 0)
                ...
                targetX = centerX+step; // 中央
            else
                targetX = centerX+step;
        }

        lane.DuringBentEndPoint = new Vector3(
            targetX,
            lane.breakPoint.y + lane.bentDir.y * Mathf.Abs((targetX - lane.breakPoint.x) / Mathf.Max(0.0001f, lane.bentDir.x)),
            lane.altEndPoint.z
        );
    ...
    Vector3[] BuildFrontHalf(Spline spline, LaneParameter lane,bool spendHalfSquare)
    {
        Vector3[] bentStack;
        bentStack = new Vector3[numberOfKnots];
        for (int i = 0; i < numberOfKnots; i++)
        {
            ...
            else
            {
                ...
                if(!spendHalfSquare)
                    posFirst = Vector3.Lerp(lane.breakPoint, lane.bentEndPoint*2, localT);
                else if(spendHalfSquare)
                    posFirst = Vector3.Lerp(lane.breakPoint, lane.DuringBentEndPoint, localT);
                VerticalChanger = false;
            }
            bentStack[i] = posFirst;
            if(!spendHalfSquare)
    ...
    private Vector3 BuildBackHalf(Spline spline,int m,bool spendaHalfSquare,int endPos,int startPos,Vector3[] InputBent)
    {
        float onePieceDis = 0;
        
        if(m==0)
            onePieceDis = (Vector3.Distance(endPoint, startPoint) / numberOfKnots) / 2f;
        else if (m == 1)
            onePieceDis = (Vector3.Distance(endPoint, startPoint) / numberOfKnots) / 2f;
        else if (m == 2)
            onePieceDis = (Vector3.Distance(endPoint, startPoint) / numberOfKnots) / 2f;
        ...
        {
            for (int count = 0; count < InputBent.Length; count++)
            {
                spline.Add(new BezierKnot(InputBent[count]));
            }
            //spline.Add(new BezierKnot(InputBent));
            rotate = new Vector3(1,0,0);

        }

        Vector3 firstTailPoint = (Vector3)(spline[numberOfKnots - 1].Position) + rotate * onePieceDis * 2;
       //Vector3 firstTailPoint = (Vector3)(spline[numberOfKnots - 1].Position)+ rotate * onePieceDis;
        Vector3 tailEnd = firstTailPoint +direction * (onePieceDis * (numberOfKnots - 1));
        if (spendaHalfSquare)
        {
    ...
    public Vector3 ReverseMatrix()
    {
        
            Debug.Log(current);
            current = new Vector3(
                current.x * toggle.x + current.z * toggle.y,
                current.y,
                current.x * toggle.y + current.z * toggle.x
            );
