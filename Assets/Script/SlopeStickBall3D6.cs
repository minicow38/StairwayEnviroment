...
public sealed class SlopeStickBall6 : MonoBehaviour
{
    const string ImplementationVersion = "CounterfactualContactController-v9-Max32-2026-07-24";
    ...
    [Tooltip("旧Controller用の接地観測猶予FixedUpdate数です。Counterfactual Controller ON時は下の秒指定を使用します。")]
    [Range(0, 12)] [SerializeField] int slopeTrackingGraceFrames = 3;
    [Tooltip("Counterfactual ControllerでSlopeFrameを保持する実時間です。120Hzなら0.06秒は約8 FixedUpdate、50Hzなら3 FixedUpdateになります。")]
    [Min(0f)] [SerializeField] float counterfactualSlopeTrackingGraceSeconds = 0.06f;
    ...
    [Header("Counterfactual Contact Controller")]
    [Tooltip("Three.js系の反実仮想物理データで重視される重力・摩擦・反発・接触継続を、単一の接線Drive、法線Stick、横センタリングへ整理して適用します。ON時は旧Target Progress加速度とSoft Governorを重ねません。")]
    [SerializeField] bool useCounterfactualContactController = true;
    [Tooltip("このControllerはdt依存を指数式で正規化しているため任意のFixed Timestepで動作します。1/120秒以外の時だけ警告します。")]
    [SerializeField] bool warnWhenFixedStepDiffersFrom120Hz = true;
    [Min(1f)] [SerializeField] float counterfactualReferenceFixedStepHz = 120f;

    [Tooltip("接地履歴の指数減衰時定数です。値が大きいほど短い接地抜けを同じ接触として扱います。")]
    [Min(0.005f)] [SerializeField] float contactMeasureTimeConstant = 0.08f;

    [Tooltip("斜面Progress=0で使用する基準速度です。")]
    [Min(0f)] [SerializeField] float counterfactualBaseSpeed = 9.2f;
    [Tooltip("log(1 + progress / scale)のscaleです。小さいほど斜面入口で速度目標が早く増えます。")]
    [Min(0.01f)] [SerializeField] float counterfactualProgressScale = 0.20f;
    [Tooltip("ONならTarget Slope ProgressでMax Ground Speedへ一致するLog Gainを自動逆算します。")]
    [SerializeField] bool autoCalibrateCounterfactualLogGain = true;
    [Tooltip("自動逆算をOFFにした場合の手動Log Gainです。")]
    [Min(0f)] [SerializeField] float counterfactualLogGain = 14f;

    [Tooltip("速度誤差を接線加速度へ変換する比例ゲインです。単位は1/秒です。")]
    [Min(0.1f)] [SerializeField] float counterfactualDriveSharpness = 4f;
    [Tooltip("接線加速度を指数平滑する時定数です。Fixed Timestepが変わっても応答時間は維持されます。")]
    [Min(0.005f)] [SerializeField] float counterfactualDriveResponseSeconds = 0.04f;

    [Tooltip("曲率モデルが無効な面で、speedRatio²へ掛ける予備Stick加速度です。")]
    [Min(0f)] [SerializeField] float counterfactualFallbackExtraStick = 220f;
    [Tooltip("接触占有率0～1へ掛ける追加Stickです。短い接地抜けでは緩やかに減衰します。")]
    [Min(0f)] [SerializeField] float counterfactualContactMeasureGain = 12f;
    [Tooltip("法線Stickを指数平滑する時定数です。")]
    [Min(0.005f)] [SerializeField] float counterfactualStickResponseSeconds = 0.04f;
    [Tooltip("面から外向きの法線速度を0へ戻す目標時間です。")]
    [Min(0.01f)] [SerializeField] float counterfactualNormalSnapResponseSeconds = 0.08f;

    [Header("Counterfactual Side Centering")]
    [Tooltip("斜面中心線からの横ずれ1m当たりの復元加速度です。")]
    [Min(0f)] [SerializeField] float sideCenteringStiffness = 12f;
    [Tooltip("横速度1m/s当たりの減衰加速度です。")]
    [Min(0f)] [SerializeField] float sideCenteringDamping = 6f;
    [Tooltip("横センタリングに使用できる最大加速度です。")]
    [Min(0f)] [SerializeField] float sideCenteringMaximumAcceleration = 20f;
    [SerializeField] bool logCounterfactualContactController = true;
    ...
    [SerializeField] float capturedAutomaticControlSeconds;
    [SerializeField] float currentOverspeedGovernorAcceleration;
    [SerializeField] float currentEffectiveAdaptiveStickSafetyLimit;

    [Header("Counterfactual Contact Runtime")]
    [SerializeField] float contactMeasureSeconds;
    [Range(0f, 1f)] [SerializeField] float contactOccupancy;
    [SerializeField] float counterfactualTargetSpeed;
    [SerializeField] float counterfactualAppliedDriveAcceleration;
    [SerializeField] float counterfactualTargetStickAcceleration;
    [SerializeField] float counterfactualAppliedStickAcceleration;
    [SerializeField] float counterfactualNormalSnapAcceleration;
    [SerializeField] float counterfactualSideError;
    [SerializeField] float counterfactualSideAcceleration;
    ...
    float previousCounterfactualDriveAcceleration;
    float previousCounterfactualStickAcceleration;
    bool counterfactualFixedStepWarningLogged;
    bool counterfactualControllerLoggedForCurrentSlope;
    Collider counterfactualLoggedSlopeCollider;
    ...
    void Awake()
    {
        ...
        ResetCounterfactualContactController(true);

        WarnAboutCounterfactualFixedStepIfNeeded();
    ...
    public void BeginPlayerAndStageTurn(float playerAngle)
    {
        ...
        turnTween = DOTween.To(() => 0f, Apply, playerAngle, turnDuration).SetEase(turnEase).SetUpdate(UpdateType.Fixed).OnComplete(() =>
            {
                Apply(playerAngle);

                heading = NormalizeFlat(Quaternion.AngleAxis(playerAngle, Vector3.up) * startHeading, startHeading);

                SetPlayerHeadingRotation(playerTarget, playerEndRotation);
            });
    }

    void ApplyStageAndPlayerOrbit(Quaternion stageTurn, bool canRotateStage, bool playerIsStageChild, Vector3 pivot, Vector3 stageStartPosition, Quaternion stageStartRotation,
        Vector3 playerStartPosition)
    {
        ...
        Vector3 playerPosition = pivot + stageTurn * (playerStartPosition - pivot);
    ...
    void SetPlayerHeadingRotation(Transform playerTarget, Quaternion rotation)
    {
        if (playerTarget == transform && rb && !rb.isKinematic)
        {
    ...
    void FixedUpdate()
    {
        ...
        UpdateCounterfactualContactMeasure(grounded, dt);
        ...
        else
        {
            ...
            int effectiveSlopeTrackingGraceFrames = GetEffectiveSlopeTrackingGraceFrames(dt);
            ...
            if (consecutiveGroundMissFrames <= effectiveSlopeTrackingGraceFrames)
            {
                if (writeRuntimeLog && logSupportSurfaceLatch)
                {
                    Debug.Log($"[SLOPE TRACKING GRACE] " + $"time={Time.fixedTime:F4}s " + $"missFrames={consecutiveGroundMissFrames} " +
                        ...
                        $"trackedSlope={(trackedSlopeCollider ? trackedSlopeCollider.name : "None")} " + $"ground=Air stickApplied=false",
                        this);
                }
            }
            else
    ...
    void SolveGround(Vector3 move, float dt)
    {
        ...
        if (useCounterfactualContactController)
        {
            SolveGroundWithCounterfactualContactController(tangentVelocity, desiredDirection, evaluationFrame, startProgress, dt);

            return;
        }

        Vector3 movementAcceleration = CalculateTangentialAcceleration(tangentVelocity, desiredDirection, currentAllowedSpeed, dt,
            useCriticalBoundaryTracking && evaluationFrame.valid,
            currentEffectiveMaximumDeceleration);
        ...
        // 旧Controllerを選択した場合も、接線制御・Governor・Adaptive Stickは
        // 同じNatural Release Weightを共有します。
        ...
        currentOverspeedGovernorAcceleration *= artificialReleaseWeight;

        currentNormalCorrection = 0f;

        float actualInwardNormalAcceleration = CalculateAdaptiveCriticalBaseStickAcceleration(evaluationFrame, startProgress, currentTangentSpeed, movementAcceleration,
                desiredDirection,
                artificialReleaseWeight,
                dt);
        ...
        UpdateCriticalRatioAtCurrentPosition(evaluationFrame, startProgress, currentTangentSpeed, actualInwardNormalAcceleration, dt);

        CompleteTargetProgressPreconditioningIfPending();
    }

    void SolveGroundWithCounterfactualContactController(Vector3 tangentVelocity, Vector3 desiredDirection, SlopeFrame evaluationFrame, float progress, float dt)
    {
        Vector3 travelTangent = GetCounterfactualTravelTangent(evaluationFrame, desiredDirection);

        if (travelTangent.sqrMagnitude < 0.000001f)
            return;

        float tangentSpeed = Mathf.Max(0f, Vector3.Dot(tangentVelocity, travelTangent));

        currentTangentSpeed = tangentSpeed;

        counterfactualTargetSpeed = CalculateCounterfactualTargetSpeed(progress, currentAllowedSpeed);

        // 旧Target Progress加速度とは重ねません。
        // Target地点はLog Gain自動逆算の校正点として使い、
        // Natural Release計画だけを斜面接触時に捕捉します。
        if (groundKind == GroundKind.Slope && evaluationFrame.valid && evaluationFrame.collider)
        {
            EnsureNaturalArtificialReleasePlan(evaluationFrame, Mathf.Max(tangentSpeed, counterfactualTargetSpeed));

            if (counterfactualLoggedSlopeCollider != evaluationFrame.collider)
            {
                counterfactualLoggedSlopeCollider = evaluationFrame.collider;

                counterfactualControllerLoggedForCurrentSlope = false;
            }
        }

        Vector3 driveAcceleration = CalculateCounterfactualDriveAcceleration(tangentVelocity, travelTangent, counterfactualTargetSpeed, dt);

        Vector3 sideAcceleration = CalculateCounterfactualSideCentering(evaluationFrame, progress, tangentVelocity, travelTangent);

        float releaseWeight = GetCurrentArtificialReleaseWeight(evaluationFrame, progress);

        driveAcceleration *= releaseWeight;
        sideAcceleration *= releaseWeight;

        currentTangentialAcceleration = counterfactualAppliedDriveAcceleration * releaseWeight;

        currentOverspeedGovernorAcceleration = tangentSpeed > maxGroundSpeed ? Mathf.Min(currentTangentialAcceleration, 0f) : 0f;

        counterfactualSideAcceleration *= releaseWeight;

        float stickAcceleration = CalculateCounterfactualStickAcceleration(evaluationFrame, progress, tangentSpeed, releaseWeight, dt);

        Vector3 totalArtificialAcceleration = driveAcceleration + sideAcceleration - groundNormal * stickAcceleration;
        ...
        UpdateCriticalRatioAtCurrentPosition(evaluationFrame, progress, tangentSpeed, stickAcceleration, dt);

        if (writeRuntimeLog && logCounterfactualContactController && groundKind == GroundKind.Slope && evaluationFrame.valid && !counterfactualControllerLoggedForCurrentSlope)
        {
            counterfactualControllerLoggedForCurrentSlope = true;

            Debug.Log($"[COUNTERFACTUAL CONTACT CONTROL ACTIVE] " + $"time={Time.fixedTime:F4}s " + $"collider={evaluationFrame.collider.name} " + $"fixedDt={dt:F8} " +
                $"progress={progress * 100f:F3}% " + $"targetSpeed={counterfactualTargetSpeed:F6} " + $"contactOccupancy={contactOccupancy:F6} " +
                $"drive={currentTangentialAcceleration:F6} " + $"stick={stickAcceleration:F6}",
                this);
        }
    }

    Vector3 GetCounterfactualTravelTangent(SlopeFrame frame, Vector3 desiredDirection)
    {
        Vector3 tangent = frame.valid ? Vector3.ProjectOnPlane(frame.axis, groundNormal) : Vector3.ProjectOnPlane(desiredDirection, groundNormal);

        if (tangent.sqrMagnitude < 0.000001f)
        {
            tangent = Vector3.ProjectOnPlane(heading, groundNormal);
        }
        ...
        Vector3 orientationReference = desiredDirection.sqrMagnitude > 0.000001f ? desiredDirection : heading;

        if (orientationReference.sqrMagnitude > 0.000001f && Vector3.Dot(tangent, orientationReference) < 0f)
        {
            tangent = -tangent;
        }
    ...
    float CalculateCounterfactualTargetSpeed(float progress, float allowedSpeed)
    {
        float maximumSpeed = Mathf.Max(0f, maxGroundSpeed);

        if (maximumSpeed <= 0f)
            return 0f;

        float baseSpeed = Mathf.Clamp(counterfactualBaseSpeed, 0f, maximumSpeed);

        float scale = Mathf.Max(0.01f, counterfactualProgressScale);

        float gain = Mathf.Max(0f, counterfactualLogGain);

        if (autoCalibrateCounterfactualLogGain)
        {
            float targetProgress = Mathf.Clamp01(targetSlopeProgressPercent * 0.01f);

            float denominator = Mathf.Log(1f + targetProgress / scale);

            gain = denominator > 0.000001f ? Mathf.Max(0f, maximumSpeed - baseSpeed) / denominator : 0f;
        }

        float normalizedProgress = Mathf.Clamp01(progress);

        float logarithmicSpeed = baseSpeed + gain * Mathf.Log(1f + normalizedProgress / scale);

        return Mathf.Clamp(logarithmicSpeed, 0f, Mathf.Min(maximumSpeed, Mathf.Max(0f, allowedSpeed)));
    }

    Vector3 CalculateCounterfactualDriveAcceleration(Vector3 tangentVelocity, Vector3 travelTangent, float targetSpeed, float dt)
    {
        float currentSpeed = Vector3.Dot(tangentVelocity, travelTangent);

        float desiredAcceleration = (targetSpeed - currentSpeed) * Mathf.Max(0.1f, counterfactualDriveSharpness);

        float accelerationLimit = Mathf.Max(0.1f, maxGroundAcceleration);

        desiredAcceleration = Mathf.Clamp(desiredAcceleration, -accelerationLimit, accelerationLimit);

        // 目標速度以上では、指数平滑の残留による
        // 正方向加速を即時に禁止します。
        if (currentSpeed >= targetSpeed && desiredAcceleration > 0f)
        {
            desiredAcceleration = 0f;
        }

        float appliedAcceleration = ExpSmooth(previousCounterfactualDriveAcceleration, desiredAcceleration, counterfactualDriveResponseSeconds, dt);

        if (currentSpeed >= targetSpeed && appliedAcceleration > 0f)
        {
            appliedAcceleration = 0f;
        }

        previousCounterfactualDriveAcceleration = appliedAcceleration;

        counterfactualAppliedDriveAcceleration = appliedAcceleration;

        return travelTangent * appliedAcceleration;
    }

    Vector3 CalculateCounterfactualSideCentering(SlopeFrame frame, float progress, Vector3 tangentVelocity, Vector3 travelTangent)
    {
        counterfactualSideError = 0f;
        counterfactualSideAcceleration = 0f;

        if (groundKind != GroundKind.Slope || !frame.valid || !frame.collider)
        {
            return Vector3.zero;
        }

        Vector3 side = Vector3.Cross(groundNormal, travelTangent);
        ...
        Vector3 centerPoint = Vector3.Lerp(frame.entryPoint, frame.exitPoint, Mathf.Clamp01(progress));

        float sideError = Vector3.Dot(rb.worldCenterOfMass - centerPoint, side);

        float sideSpeed = Vector3.Dot(tangentVelocity, side);

        float sideAcceleration = -sideError * Mathf.Max(0f, sideCenteringStiffness) - sideSpeed * Mathf.Max(0f, sideCenteringDamping);

        sideAcceleration = Mathf.Clamp(sideAcceleration, -Mathf.Max(0f, sideCenteringMaximumAcceleration), Mathf.Max(0f, sideCenteringMaximumAcceleration));

        counterfactualSideError = sideError;

        counterfactualSideAcceleration = sideAcceleration;

        return side * sideAcceleration;
    }

    float CalculateCounterfactualStickAcceleration(SlopeFrame frame, float progress, float tangentSpeed, float releaseWeight, float dt)
    {
        counterfactualTargetStickAcceleration = 0f;
        counterfactualNormalSnapAcceleration = 0f;
        adaptiveStickSaturated = false;

        if (naturalMotionReleased || releaseWeight <= 0f)
        {
            previousCounterfactualStickAcceleration = 0f;
            counterfactualAppliedStickAcceleration = 0f;
            ...
            currentEffectiveAdaptiveStickSafetyLimit = 0f;
            return 0f;
        }

        float curvature = 0f;
        float gravitySupport = Mathf.Max(0f, Vector3.Dot(Physics.gravity, -groundNormal));

        if (groundKind == GroundKind.Slope && frame.valid && TryEvaluateSurface(frame, progress, out SurfaceSample sample))
        {
            curvature = Mathf.Max(0f, sample.curvature);

            gravitySupport = Mathf.Max(0f, Vector3.Dot(Physics.gravity, -groundNormal));
        }

        float safeMaximumSpeed = Mathf.Max(0.01f, maxGroundSpeed);

        float speedRatio = Mathf.Clamp(tangentSpeed / safeMaximumSpeed, 0f, 1.5f);

        float speedRatioSquared = speedRatio * speedRatio;

        // extraStick * speedRatio² のextraStickを、
        // 曲率が有効な斜面ではMax Ground Speedに必要な
        // 法線加速度から自動逆算します。
        float extraStickAtMaximumSpeed = 0f;

        if (groundKind == GroundKind.Slope)
        {
            extraStickAtMaximumSpeed = curvature > minimumCurvature ? CalculateRequiredBaseStickForSpeed(safeMaximumSpeed, curvature, gravitySupport) : Mathf.Max(0f,
                        counterfactualFallbackExtraStick);
        }

        float speedStick = extraStickAtMaximumSpeed * speedRatioSquared;

        float contactStick = Mathf.Max(0f, counterfactualContactMeasureGain) * contactOccupancy;

        float outwardNormalSpeed = Mathf.Max(0f, Vector3.Dot(rb.velocity, groundNormal));

        float normalSnap = outwardNormalSpeed / Mathf.Max(0.01f, counterfactualNormalSnapResponseSeconds);

        float targetStick = Mathf.Max(0f, baseStickAcceleration) + speedStick + contactStick + normalSnap;

        float safetyLimit = curvature > minimumCurvature ? GetEffectiveAdaptiveStickSafetyLimit(curvature, gravitySupport, Mathf.Max(safeMaximumSpeed, tangentSpeed)) : Mathf.Max(
                    maximumAdaptiveBaseStickAcceleration,
                    baseStickAcceleration + counterfactualFallbackExtraStick + counterfactualContactMeasureGain);

        if (useAutomaticSpeedHomogeneousControl)
        {
            safetyLimit = Mathf.Min(safetyLimit, Mathf.Max(0.1f, automaticAdaptiveStickHardLimit));
        }

        float clampedTargetStick = Mathf.Min(targetStick, safetyLimit);

        float releasedTargetStick = clampedTargetStick * releaseWeight;

        float appliedStick = ExpSmooth(previousCounterfactualStickAcceleration, releasedTargetStick, counterfactualStickResponseSeconds, dt);

        // Natural ReleaseのEnvelopeを指数平滑の残留値が
        // 上回らないよう、現在時点の容量でも制限します。
        appliedStick = Mathf.Clamp(appliedStick, 0f, safetyLimit * releaseWeight);

        previousCounterfactualStickAcceleration = appliedStick;

        counterfactualTargetStickAcceleration = targetStick;

        counterfactualAppliedStickAcceleration = appliedStick;

        counterfactualNormalSnapAcceleration = normalSnap;

        currentNormalCorrection = normalSnap;

        currentAdaptivePredictedSpeed = tangentSpeed;

        currentAdaptiveDesiredBaseStickAcceleration = targetStick;

        currentAdaptiveStickCapacity = safetyLimit * releaseWeight;

        currentAdaptiveOutwardDampingAcceleration = normalSnap;

        currentEffectiveBaseStickAcceleration = appliedStick;

        currentEffectiveAdaptiveStickSafetyLimit = safetyLimit;

        adaptiveStickSaturated = targetStick >
            safetyLimit + 0.001f;

        if (adaptiveStickSaturated)
        {
            if (!adaptiveStickSaturationLogged && writeRuntimeLog && logCounterfactualContactController)
            {
                adaptiveStickSaturationLogged = true;

                Debug.LogWarning($"[COUNTERFACTUAL STICK SATURATED] " + $"time={Time.fixedTime:F4}s " + $"collider={(frame.collider ? frame.collider.name : "None")} " +
                    $"progress={progress * 100f:F3}% " + $"speed={tangentSpeed:F6} " + $"targetStick={targetStick:F6} " + $"capacity={safetyLimit:F6} " +
                    $"contactOccupancy={contactOccupancy:F6}",
                    this);
            }
        }
        else
    ...
    int GetEffectiveSlopeTrackingGraceFrames(float dt)
    {
        if (!useCounterfactualContactController)
            return Mathf.Max(0, slopeTrackingGraceFrames);

        return Mathf.Max(0, Mathf.CeilToInt(Mathf.Max(0f, counterfactualSlopeTrackingGraceSeconds) / Mathf.Max(0.000001f, dt)));
    }

    void UpdateCounterfactualContactMeasure(bool grounded, float dt)
    {
        if (!useCounterfactualContactController)
            return;

        float tau = Mathf.Max(0.005f, contactMeasureTimeConstant);

        float decay = Mathf.Exp(-dt / tau);

        // ユーザー式:
        // measure[n+1] = measure[n] exp(-dt/tau) + grounded * dt
        contactMeasureSeconds = contactMeasureSeconds * decay + (grounded ? dt : 0f);

        // 上式の離散定常値 dt / (1 - exp(-dt/tau)) で割り、
        // Fixed Timestepに依存しない0～1の占有率へ変換します。
        float discreteSteadyState = dt / Mathf.Max(0.000001f, 1f - decay);

        contactOccupancy = Mathf.Clamp01(contactMeasureSeconds / discreteSteadyState);

        if (!grounded)
        {
            previousCounterfactualDriveAcceleration = ExpSmooth(previousCounterfactualDriveAcceleration, 0f, counterfactualDriveResponseSeconds, dt);

            previousCounterfactualStickAcceleration = ExpSmooth(previousCounterfactualStickAcceleration, 0f, counterfactualStickResponseSeconds, dt);
        }
    }

    void WarnAboutCounterfactualFixedStepIfNeeded()
    {
        if (!useCounterfactualContactController || !warnWhenFixedStepDiffersFrom120Hz || counterfactualFixedStepWarningLogged)
        {
            return;
        }

        float referenceDt = 1f / Mathf.Max(1f, counterfactualReferenceFixedStepHz);

        if (Mathf.Abs(Time.fixedDeltaTime - referenceDt) <= 0.00001f)
        {
            return;
        }

        counterfactualFixedStepWarningLogged = true;

        Debug.LogWarning($"[COUNTERFACTUAL FIXED STEP] " + $"current={Time.fixedDeltaTime:F8}s " + $"reference={referenceDt:F8}s. " +
            $"Controller formulas are dt-normalized, but Project Settings > Time > Fixed Timestep = {referenceDt:F8} gives the intended 120Hz resolution.",
            this);
    }

    void ResetCounterfactualContactController(bool resetContactMeasure)
    {
        if (resetContactMeasure)
        {
            contactMeasureSeconds = 0f;
            contactOccupancy = 0f;
        }

        previousCounterfactualDriveAcceleration = 0f;
        previousCounterfactualStickAcceleration = 0f;
        counterfactualTargetSpeed = 0f;
        counterfactualAppliedDriveAcceleration = 0f;
        counterfactualTargetStickAcceleration = 0f;
        counterfactualAppliedStickAcceleration = 0f;
        counterfactualNormalSnapAcceleration = 0f;
        counterfactualSideError = 0f;
        counterfactualSideAcceleration = 0f;
        counterfactualControllerLoggedForCurrentSlope = false;
        counterfactualLoggedSlopeCollider = null;
        adaptiveStickSaturationLogged = false;
    }

    static float ExpSmooth(float current, float target, float responseSeconds, float dt)
    {
        float safeResponse = Mathf.Max(0.000001f, responseSeconds);

        float alpha = 1f - Mathf.Exp(-Mathf.Max(0f, dt) / safeResponse);

        return Mathf.LerpUnclamped(current, target, alpha);
    }

    void CaptureAutomaticSpeedScaledLimits(SlopeFrame frame, float currentProgress, float targetProgress, float targetSpeed)
    {
        ...
        capturedAutomaticControlSeconds = 0f;
        ...
        capturedAutomaticControlSeconds = Mathf.Max(controlDistance / Mathf.Max(averageSpeed, 0.1f), Time.fixedDeltaTime * 2f);
        ...
        float requiredNetAcceleration = (safeTargetSpeed * safeTargetSpeed - safeCurrentSpeed * safeCurrentSpeed) / (2f * controlDistance);
        ...
        float riseSeconds = Mathf.Clamp(capturedAutomaticControlSeconds * 0.35f, 0.06f, 0.18f);
        ...
        if (writeRuntimeLog && logAutomaticSpeedHomogeneousControl)
        {
            Debug.Log($"[AUTOMATIC SPEED CONTROL CAPTURED] time={Time.fixedTime:F4}s " + $"collider={(frame.collider ? frame.collider.name : "None")} " +
                ...
                $"controlDistance={controlDistance:F6} controlSeconds={capturedAutomaticControlSeconds:F6} " + $"positiveLimit={capturedAutomaticPositiveAccelerationLimit:F6} " +
        ...
        float dt)
    {
        ...
        targetProgressDesiredArtificialAcceleration = Mathf.Clamp(targetProgressDesiredArtificialAcceleration, -Mathf.Max(0.1f, negativeLimit), Mathf.Max(0f, positiveLimit));

        // 目標速度以上では正方向の人工加速度を禁止し、速度超過を増幅させません。
        if (currentSpeed >= capturedTargetTangentSpeed)
            targetProgressDesiredArtificialAcceleration = Mathf.Min(targetProgressDesiredArtificialAcceleration, 0f);
    ...
    {
        currentOverspeedGovernorAcceleration = 0f;
        ...
        {
            previousOverspeedGovernorAcceleration = Mathf.MoveTowards(previousOverspeedGovernorAcceleration, 0f, Mathf.Max(0.1f, effectiveJerkLimit) * dt);

            currentOverspeedGovernorAcceleration = previousOverspeedGovernorAcceleration;
        ...
        float requiredGovernorAcceleration = (maxGroundSpeed * maxGroundSpeed - currentSpeed * currentSpeed) / (2f * recoveryDistance);
        ...
        previousOverspeedGovernorAcceleration = Mathf.MoveTowards(previousOverspeedGovernorAcceleration, requiredGovernorAcceleration, Mathf.Max(0.1f, effectiveJerkLimit) * dt);
    ...
    bool EnsureTargetProgressPreconditioningPlan(SlopeFrame evaluationFrame, float currentProgress)
    {
        ...
        CaptureAutomaticSpeedScaledLimits(evaluationFrame, currentProgress, targetProgress, capturedTargetTangentSpeed);
    ...
    float GetEffectiveAdaptiveStickSafetyLimit(float curvature, float gravitySupport, float designSpeed)
    {
        ...
        {
            currentEffectiveAdaptiveStickSafetyLimit = manualLimit;
            return manualLimit;
        }
        ...
        currentEffectiveAdaptiveStickSafetyLimit = effectiveLimit;
        ...
        if (adaptiveStickSaturated)
        {
            if (!adaptiveStickSaturationLogged && writeRuntimeLog && logAdaptiveCriticalBaseStick)
            {
                ...
                Debug.LogWarning($"[ADAPTIVE CRITICAL STICK SATURATED] time={Time.fixedTime:F4}s " + $"collider={frame.collider.name} progress={progress * 100f:F3}% " +
                    ...
                    $"capacity={capacity:F6} maxGroundSpeed={maxGroundSpeed:F6} " + $"maximumAdaptive={maximumAdaptiveBaseStickAcceleration:F6} effectiveSafetyLimit={currentEffectiveAdaptiveStickSafetyLimit:F6}", this);
            }
        }
        else
        {
    ...
    void WriteLogIfNeeded()
    {
        ...
        Debug.Log($"[CRITICAL TRACE] time={Time.fixedTime:F4}s " + $"ground={groundKind} slope={activeSlopeName} forwardSlope={hasForwardSlope} " +
            ...
            $"autoPositiveLimit={capturedAutomaticPositiveAccelerationLimit:F6} " + $"autoNegativeLimit={capturedAutomaticNegativeAccelerationLimit:F6} " +
            $"autoJerkLimit={capturedAutomaticJerkLimit:F6} " + $"overspeedGovernor={currentOverspeedGovernorAcceleration:F6} " +
            $"adaptiveSafetyLimit={currentEffectiveAdaptiveStickSafetyLimit:F6} " +
            ...
            $"adaptiveStickSaturated={adaptiveStickSaturated} " + $"counterfactualMode={useCounterfactualContactController} " +
            $"contactMeasure={contactMeasureSeconds:F6} contactOccupancy={contactOccupancy:F6} " + $"counterfactualTargetSpeed={counterfactualTargetSpeed:F6} " +
            $"counterfactualDrive={counterfactualAppliedDriveAcceleration:F6} " + $"counterfactualTargetStick={counterfactualTargetStickAcceleration:F6} " +
            $"counterfactualStick={counterfactualAppliedStickAcceleration:F6} " + $"counterfactualNormalSnap={counterfactualNormalSnapAcceleration:F6} " +
            $"sideError={counterfactualSideError:F6} sideAcceleration={counterfactualSideAcceleration:F6} " + $"naturalMotionReleased={naturalMotionReleased}", this);
    }
