...
public sealed class SlopeStickBall3D11 : MonoBehaviour
{
    const string ImplementationVersion = "AdaptiveCriticalAdhesion-NaturalRelease-SafePredictiveControl-v7-2026-07-23";
    enum GroundKind
    {
        Air,
        Flat,
    ...
    [Tooltip("プレイヤーの向きを表示・回転するTransformです。未設定ならこのGameObjectを使用します。")]
    ...
    [Range(1, 60)] [SerializeField] int requiredStableSlopeFrames = 2;
    ...
    void ReadTurnFlick()
    {
        ...
        BeginPlayerAndStageTurn(flick.x > 0f ? turnAngle : -turnAngle);
    }
    void BeginPlayerAndStageTurn(float playerAngle)
    {
        FindStageTurnReferences();
        ...
        Quaternion playerEndRotation =
            Quaternion.AngleAxis(playerAngle, Vector3.up) * playerStartRotation;
        bool canRotateStage = stageRoot && stagePivot;
        bool playerIsStageChild =
            canRotateStage && transform.IsChildOf(stageRoot);
        Vector3 pivot = canRotateStage ? stagePivot.position : Vector3.zero;
        Vector3 stageStartPosition =
            canRotateStage ? stageRoot.position : Vector3.zero;
        Quaternion stageStartRotation =
            canRotateStage ? stageRoot.rotation : Quaternion.identity;
        ...
        void Apply(float angle)
        {
            ...
            Quaternion stageTurn =
                Quaternion.AngleAxis(angle * stageMultiplier, Vector3.up);
            ...
            if (consecutiveGroundMissFrames <= slopeTrackingGraceFrames)
            {
                if (writeRuntimeLog && logSupportSurfaceLatch)
                {
                    Debug.Log($"[SLOPE TRACKING GRACE] time={Time.fixedTime:F4}s " + $"missFrames={consecutiveGroundMissFrames} " + $"graceFrames={slopeTrackingGraceFrames} " +
                        $"trackedSlope={(trackedSlopeCollider ? trackedSlopeCollider.name : "None")} " +
                        $"ground=Air stickApplied=false", this);
                }
            }
            else
            {
        ...
        if (wasGrounded && !grounded)
        {
            ...
            if (writeRuntimeLog)
            {
                ...
                Debug.LogWarning($"[CRITICAL AIR TRANSITION] judgement={judgement} " + $"time={Time.fixedTime:F4}s " + $"progress={currentSlopeProgressPercent:F3}% " +
                    $"ratio={lastCriticalRatioBeforeAir:F6} " + $"target={targetCriticalRatio:F6} " +
                    $"error={Mathf.Abs(lastCriticalRatioBeforeAir - targetCriticalRatio):F6} " + $"speed={currentTangentSpeed:F6} " + $"allowed={currentAllowedSpeed:F6} " +
                    $"localCritical={currentLocalCriticalSpeed:F6} " + $"curvature={currentCurvature:F8} " +
                    $"available={currentAvailableNormalAcceleration:F6} " + $"required={currentRequiredNormalAcceleration:F6} " +
                    $"maintainedBeforeAir={previousLoggedCriticalMaintained}", this);
            }
        }
    ...
    void UpdateSupportObservationDebug(bool grounded, GroundObservation observation)
    {
        ...
        Debug.Log($"[SUPPORT OBSERVATION] time={Time.fixedTime:F4}s " + $"grounded={grounded} " + $"source={currentSource} " +
            $"collider={(currentCollider ? currentCollider.name : "None")} " +
            $"point={(grounded ? observation.point.ToString("F5") : "N/A")} " + $"normal={(grounded ? observation.normal.ToString("F5") : "N/A")}", this);
    ...
    bool BuildSlopeFrame(Collider slopeCollider, Vector3 referenceNormalInput, Vector3 desiredForward, out SlopeFrame frame)
    {
        ...
        if (endpointNormalDifference > sameSectionNormalTolerance)
        {
            if (writeRuntimeLog)
            {
                Debug.LogWarning($"[FRAME SECTION REJECTED] collider={slopeCollider.name} " + $"entryPoint={entryPoint:F5} " + $"exitPoint={exitPoint:F5} " +
                    $"entryNormal={entryNormal:F5} " + $"exitNormal={exitNormal:F5} " +
                    $"normalDifference={endpointNormalDifference:F4} " + $"limit={sameSectionNormalTolerance:F4}", this);
            }
            frame = default;
            return false;
        }
        ...
        if (axisNormalDot > 0.001f || Mathf.Abs(endpointAngle - axisAngle) > endpointDirectionTolerance)
        {
            if (writeRuntimeLog)
            {
                Debug.LogWarning($"[FRAME SECTION REJECTED] collider={slopeCollider.name} " + $"reason=AxisOrEndpointAngleMismatch " + $"surfaceAngle={surfaceAngle:F4} " +
                    $"axisAngle={axisAngle:F4} " + $"endpointAngle={endpointAngle:F4} " +
                    $"axisNormalDot={axisNormalDot:F6} " + $"entryPoint={entryPoint:F5} " + $"exitPoint={exitPoint:F5}", this);
            }
            frame = default;
            return false;
        }
        ...
        if (writeRuntimeLog)
        {
            Debug.Log($"[FRAME SECTION APPROVED] collider={slopeCollider.name} " + $"entryPoint={entryPoint:F5} " + $"exitPoint={exitPoint:F5} " +
                $"entryNormal={entryNormal:F5} " + $"exitNormal={exitNormal:F5} " +
                $"normalDifference={endpointNormalDifference:F4} " + $"surfaceAngle={surfaceAngle:F4} " + $"axisAngle={axisAngle:F4} " + $"endpointAngle={endpointAngle:F4} " +
                $"axisNormalDot={axisNormalDot:F6} " + $"length={frame.projectedLength:F6} " +
                $"entryConnected={(frame.entryConnectedCollider ? frame.entryConnectedCollider.name : "None")} " +
                $"exitConnected={(frame.exitConnectedCollider ? frame.exitConnectedCollider.name : "None")} " +
                $"entryBoundaryCurvature={frame.entryBoundaryCurvature:F8} " + $"exitBoundaryCurvature={frame.exitBoundaryCurvature:F8} " +
                $"representativeCurvature={frame.representativeCurvature:F8}", this);
        }
    ...
    void PopulateRepresentativeSectionCurvature(ref SlopeFrame frame, Vector3 entryNormal, Vector3 exitNormal)
    {
        ...
        if (TrySampleConnectedSurface(frame, frame.exitPoint, 1f, out Vector3 exitConnectedPoint, out Vector3 exitConnectedNormal, out Collider exitConnectedCollider))
        {
            ...
            frame.exitBoundaryCurvature = CalculateBoundaryCurvature(exitNormal, exitConnectedNormal, frame.axis, Vector3.Distance(frame.exitPoint, exitConnectedPoint),
                exitNormal);
        }

        frame.representativeCurvature = Mathf.Clamp(Mathf.Max(frame.entryBoundaryCurvature, frame.exitBoundaryCurvature), 0f, Mathf.Max(minimumCurvature,
    ...
    void SolveGround(Vector3 move, float dt)
    {
        ...
        Vector3 movementAcceleration = CalculateTangentialAcceleration(tangentVelocity, desiredDirection, currentAllowedSpeed, dt, useCriticalBoundaryTracking &&
            evaluationFrame.valid, currentEffectiveMaximumDeceleration);
        ...
        float actualInwardNormalAcceleration = CalculateAdaptiveCriticalBaseStickAcceleration(evaluationFrame, startProgress, forwardSpeed, movementAcceleration, desiredDirection,
            artificialReleaseWeight, dt);
        ...
        float currentProgress, float dt)
    {
        ...
        // 既存のInspector上下限を実際のTarget Progress制御へ適用します。
        float minimumArtificialAcceleration =
            -Mathf.Max(0.1f, targetProgressMaximumArtificialDeceleration);

        // 法線需要が臨界値へ近づくほど、正方向の人工加速だけを連続的に縮小します。
        // 減速側は安全確保に必要なため縮小しません。
        float normalHeadroom =
            Mathf.Clamp01(
                (targetCriticalRatio - currentNormalDemandRatio) /
                Mathf.Max(targetCriticalRatio, 0.0001f));

        float maximumArtificialAcceleration =
            Mathf.Max(0f, targetProgressMaximumArtificialAcceleration) *
            normalHeadroom;

        // 次のFixedUpdateで現在地点のAllowed Speedを超えない範囲まで、
        // 正方向加速度の上限を予測的に狭めます。
        float maximumSafeNetAcceleration =
            (currentAllowedSpeed - currentSpeed) /
            Mathf.Max(dt, 0.000001f);

        float maximumSafeArtificialAcceleration =
            maximumSafeNetAcceleration -
            targetProgressGravityAcceleration;

        maximumArtificialAcceleration =
            Mathf.Min(
                maximumArtificialAcceleration,
                maximumSafeArtificialAcceleration);

        // すでにAllowed Speedを超えている場合は、上限を負側まで下げて
        // 少なくとも必要な制動を要求できるようにします。
        maximumArtificialAcceleration =
            Mathf.Max(
                minimumArtificialAcceleration,
                maximumArtificialAcceleration);

        targetProgressDesiredArtificialAcceleration =
            Mathf.Clamp(
                targetProgressRequiredNetAcceleration -
                targetProgressGravityAcceleration,
                minimumArtificialAcceleration,
                maximumArtificialAcceleration);

        // 過去の範囲外状態をJerk制御へ持ち越さないよう、前回値も先に制限します。
        float boundedPreviousAcceleration =
            Mathf.Clamp(
                previousTargetProgressAppliedAcceleration,
                minimumArtificialAcceleration,
                maximumArtificialAcceleration);

        targetProgressAppliedArtificialAcceleration =
            Mathf.MoveTowards(
                boundedPreviousAcceleration,
                targetProgressDesiredArtificialAcceleration,
    ...
    bool EnsureTargetProgressPreconditioningPlan(SlopeFrame evaluationFrame, float currentProgress)
    {
        ...
        if (writeRuntimeLog && logTargetProgressPreconditioning)
        {
            Debug.Log($"[TARGET PROGRESS PLAN CAPTURED] time={Time.fixedTime:F4}s " + $"collider={evaluationFrame.collider.name} " +
                $"currentProgress={currentProgress * 100f:F3}% " + $"targetProgress={capturedTargetProgressPercent:F3}% " +
                $"targetSpeed={capturedTargetTangentSpeed:F6} " + $"targetCurvature={capturedTargetCurvature:F8} " +
                $"targetAvailable={capturedTargetAvailableNormalAcceleration:F6} " + $"targetRatio={targetCriticalRatio:F6} " + $"additionalAddForce=false", this);
        }
    ...
    void CompleteTargetProgressPreconditioningIfPending()
    {
        ...
        if (writeRuntimeLog && logTargetProgressPreconditioning)
        {
            Debug.Log($"[TARGET PROGRESS PRECONDITION COMPLETED] time={Time.fixedTime:F4}s " +
                ...
                $"actualRatio={currentCriticalRatio:F6} " + $"targetRatio={targetCriticalRatio:F6} " +
                $"targetControlReleased=true " + $"naturalReleaseWeight={currentArtificialReleaseWeight:F6} " + $"naturalMotionReleased={naturalMotionReleased} pullback=false",
                this);
        }
    }
    ...
    void UpdateNaturalArtificialReleaseState(SlopeFrame evaluationFrame, float currentProgress)
    {
        ...
        if (!naturalReleaseLatchLogged && writeRuntimeLog && logNaturalArtificialRelease)
        {
            ...
            Debug.Log($"[NATURAL ARTIFICIAL RELEASE LATCHED] time={Time.fixedTime:F4}s " + $"collider={(naturalReleasePlanCollider ? naturalReleasePlanCollider.name : "None")} " +
                $"progress={progressPercent:F3}% " +
                $"releasePoint={capturedNaturalReleaseProgressPercent:F3}% " + $"baseStick=0 tangentialArtificial=0 airArtificial=0 " +
    ...
    bool EnsureNaturalArtificialReleasePlan(SlopeFrame frame, float speedCandidate)
    {
        ...
        if (availableWidth <= 0.0001f && writeRuntimeLog && logNaturalArtificialRelease)
        {
            Debug.LogWarning($"[NATURAL RELEASE CONFIG INVALID] targetProgress={targetSlopeProgressPercent:F3}% " + $"hold={releaseHoldAfterTargetPercent:F3}% " +
                $"releaseEnd={releaseEnd:F3}% reason=ReleaseIntervalIsZero " +
                $"recommended=Target30_Release50", this);
        }
        ...
        if (writeRuntimeLog && logNaturalArtificialRelease)
        {
            Debug.Log($"[NATURAL ARTIFICIAL RELEASE PLAN CAPTURED] time={Time.fixedTime:F4}s " + $"collider={frame.collider.name} " +
                $"targetProgress={targetSlopeProgressPercent:F3}% " + $"fullStickUntil={minimumStart:F3}% " +
                $"releaseStart={capturedReleaseStartProgressPercent:F3}% " + $"releaseEnd={capturedNaturalReleaseProgressPercent:F3}% " +
                $"designSpeed={capturedReleaseDesignSpeed:F6} " + $"maxGroundSpeed={maxGroundSpeed:F6} " +
        ...
        if (adaptiveStickSaturated)
        {
            if (!adaptiveStickSaturationLogged && writeRuntimeLog && logAdaptiveCriticalBaseStick)
            {
                ...
                Debug.LogWarning($"[ADAPTIVE CRITICAL STICK SATURATED] time={Time.fixedTime:F4}s " + $"collider={frame.collider.name} progress={progress * 100f:F3}% " +
                    $"speed={currentSpeed:F6} predictedSpeed={predictedSpeed:F6} " +
                    $"required={requiredStick:F6} damping={outwardDampingAcceleration:F6} " + $"capacity={capacity:F6} maxGroundSpeed={maxGroundSpeed:F6} " +
    ...
    Vector3 CalculateTangentialAcceleration(Vector3 tangentVelocity, Vector3 desiredDirection, float allowedSpeed, float dt, bool criticalControlActive, float maximumDeceleration)
    {
        ...
        {
            // 内部状態には、実際に返すClamp後の加速度だけを保存します。
        ...
        float decelerationLimit =
            Mathf.Max(0.1f, maximumDeceleration);

        // Critical Controlへ入る前の状態が大きくても、物理上限内から開始します。
        currentTangentialAcceleration =
            Mathf.Clamp(
                currentTangentialAcceleration,
                -decelerationLimit,
                maximumCriticalAcceleration);
        ...
        float desiredForwardAcceleration =
            speedError /
            Mathf.Max(
                criticalResponseSeconds,
                dt);

        desiredForwardAcceleration =
            Mathf.Clamp(
                desiredForwardAcceleration,
                -decelerationLimit,
                maximumCriticalAcceleration);
        ...
        currentTangentialAcceleration =
            Mathf.MoveTowards(
                currentTangentialAcceleration,
                desiredForwardAcceleration,
                Mathf.Max(0.1f, criticalJerkLimit) * dt);

        // 横速度を1 FixedUpdateで消さず、既存のCritical応答時間で滑らかに減衰します。
        // 新しいレーン基準や位置復元力は追加しません。
        ...
        return
            desiredDirection * currentTangentialAcceleration +
            lateralAcceleration;
    }
    ...
    bool TryEvaluateSurface(SlopeFrame frame, float progress, out SurfaceSample sample)
    {
        ...
        else
        {
            ...
            if (!Mathf.Approximately(previousProgress, nextProgress) && TrySampleAtProgress(frame, previousProgress, out Vector3 previousPoint, out Vector3 previousNormal) &&
                TrySampleAtProgress(frame, nextProgress, out Vector3 nextPoint,
                out Vector3 nextNormal))
            {
        ...
        SurfaceSamplePurpose purpose)
    {
        ...
        RaycastHit bestHit = default;
        bool visualizeThisCall = showEntryExitRayLines && (purpose == SurfaceSamplePurpose.Entry || purpose == SurfaceSamplePurpose.Exit) && ShouldCaptureSlopeRay(frame.collider,
            purpose);
    ...
    void UpdateSlopeProgressObservation(bool grounded, GroundObservation observation, float dt)
    {
        ...
        if (!sameSession)
        {
            ...
            if (writeRuntimeLog && logProgressTargetCrossing)
            {
                Debug.Log($"[SLOPE SESSION START] time={Time.fixedTime:F4}s " + $"collider={observation.collider.name} " +
                    $"progress={current:F3}% target={targetSlopeProgressPercent:F3}% " + $"error={error:F3}% side={slopeProgressSide} " +
                    $"source={observation.source}", this);
            }
        ...
        else
        {
            ...
            if (writeRuntimeLog && logProgressTargetCrossing)
            {
                Debug.LogWarning($"[PROGRESS OBSERVATION RESET] time={Time.fixedTime:F4}s " + $"collider={observation.collider.name} " +
                    $"previous={previousSlopeProgressPercent:F3}% " + $"current={current:F3}% " + $"delta={slopeProgressDeltaPercent:F3}%",
                    this);
            }
        }
        ...
        if (readyForLimitCapture && !limitCaptureReadyLogged)
        {
            ...
            if (writeRuntimeLog && logProgressTargetCrossing)
            {
                Debug.Log($"[LIMIT CAPTURE OBSERVATION READY] time={Time.fixedTime:F4}s " + $"collider={observation.collider.name} " + $"stableFrames={stableSlopeContactFrames} " +
                    ...
                    $"error={error:F3}% side={slopeProgressSide} " + $"forcesChanged=false", this);
            }
        }
        ...
        if (crossedTargetThisFrame)
        {
            ...
            if (writeRuntimeLog && logProgressTargetCrossing)
            {
                ...
                Debug.Log($"[PROGRESS TARGET CROSSED] time={Time.fixedTime:F4}s " + $"estimatedCrossingTime={estimatedTargetCrossingTime:F6}s " +
                    $"collider={observation.collider.name} " + $"previous={previousSlopeProgressPercent:F3}% " +
                    $"current={current:F3}% " + $"target={targetSlopeProgressPercent:F3}% " + $"previousError={previousSlopeProgressErrorPercent:F3}% " +
                    $"currentError={error:F3}% " + $"direction={direction} " + $"targetPhase={targetProgressPhase} " +
    ...
    void UpdateCriticalDebugEvents(bool grounded)
    {
        ...
        bool criticalControlActive = grounded && useCriticalBoundaryTracking && ((groundKind == GroundKind.Slope && slopeFrame.valid) || (groundKind == GroundKind.Flat &&
            hasForwardSlope && forwardSlopeFrame.valid));
        ...
        if (criticalControlActive && !previousLoggedCriticalControlActive)
        {
            Debug.Log($"[CRITICAL CONTROL START] time={Time.fixedTime:F4}s " + $"ground={groundKind} slope={activeSlopeName} " + $"progress={currentSlopeProgressPercent:F3}% " +
                $"controlProgress={controllingSampleProgressPercent:F3}% " +
        ...
        if (inTolerance && !previousLoggedInCriticalTolerance)
        {
            Debug.Log($"[CRITICAL ENTER] time={Time.fixedTime:F4}s " + $"progress={currentSlopeProgressPercent:F3}% " +
                ...
                $"error={Mathf.Abs(currentCriticalRatio - targetCriticalRatio):F6} " + $"speed={currentTangentSpeed:F6} " + $"localCritical={currentLocalCriticalSpeed:F6} " +
                $"curvature={currentCurvature:F8}", this);
        }
        ...
        if (logCriticalSuccess && !criticalStateMaintained && previousLoggedCriticalMaintained)
        {
            Debug.LogWarning($"[CRITICAL MAINTENANCE LOST] time={Time.fixedTime:F4}s " + $"slope={activeSlopeName} " + $"progress={currentSlopeProgressPercent:F3}% " +
                $"ratio={currentCriticalRatio:F6} target={targetCriticalRatio:F6} " +
                $"error={Mathf.Abs(currentCriticalRatio - targetCriticalRatio):F6} " + $"speed={currentTangentSpeed:F6} allowed={currentAllowedSpeed:F6}", this);
        }
    ...
    void CacheDebugState()
    {
        ...
        previousLoggedCriticalControlActive = useCriticalBoundaryTracking && ((groundKind == GroundKind.Slope && slopeFrame.valid) || (groundKind == GroundKind.Flat &&
            hasForwardSlope && forwardSlopeFrame.valid));
    ...
    void OnDisable()
    {
        ...
        Debug.Log($"[CRITICAL SESSION SUMMARY] successes={sessionCriticalSuccessCount} " + $"groundToAir={sessionGroundToAirCount} " +
            $"slopeDetections={sessionSlopeDetectionCount} " + $"maxRatio={sessionMaximumCriticalRatio:F6} " +
            $"minTargetError={minimumErrorText} " + $"longestMaintained={sessionLongestMaintainedSeconds:F4}s " + $"targetRatio={targetCriticalRatio:F6} " +
    ...
    void WriteLogIfNeeded()
    {
        ...
        Debug.Log($"[CRITICAL TRACE] time={Time.fixedTime:F4}s " + $"ground={groundKind} slope={activeSlopeName} forwardSlope={hasForwardSlope} " +
            ...
            $"speed={currentTangentSpeed:F6} allowed={currentAllowedSpeed:F6} " + $"localCritical={currentLocalCriticalSpeed:F6} " + $"curvature={currentCurvature:F8} " +
            $"sectionCurvature={currentRepresentativeCurvature:F8} " +
            $"effectiveDeceleration={currentEffectiveMaximumDeceleration:F6} " + $"normalDemandRatio={currentNormalDemandRatio:F6} " +
            $"available={currentAvailableNormalAcceleration:F6} " + $"required={currentRequiredNormalAcceleration:F6} " +
            $"ratio={currentCriticalRatio:F6} targetRatio={targetCriticalRatio:F6} " + $"tangentAcceleration={currentTangentialAcceleration:F6} " +
            $"normalCorrection={currentNormalCorrection:F6} " +
            ...
            $"supportCollider={currentGroundColliderName} " + $"targetProgress={targetSlopeProgressPercent:F3}% " +
            $"progressError={slopeProgressErrorPercent:F3}% " + $"progressSide={slopeProgressSide} " + $"progressDelta={slopeProgressDeltaPercent:F3}% " +
            $"stableSlopeFrames={stableSlopeContactFrames} " + $"observationValid={slopeProgressObservationValid} " +
            $"crossedTarget={crossedTargetThisFrame} " + $"readyForLimitCapture={readyForLimitCapture} " + $"targetPhase={targetProgressPhase} " +
            ...
            $"targetGravityAcceleration={targetProgressGravityAcceleration:F6} " +
            $"targetArtificialAcceleration={targetProgressAppliedArtificialAcceleration:F6} " + $"naturalReleasePlanValid={naturalReleasePlanValid} " +
            ...
            $"cumulativeRelease={currentCumulativeReleaseRatio:F6} " +
            $"adaptivePredictedSpeed={currentAdaptivePredictedSpeed:F6} " + $"adaptiveDesiredStick={currentAdaptiveDesiredBaseStickAcceleration:F6} " +
            $"adaptiveStickCapacity={currentAdaptiveStickCapacity:F6} " +
            ...
            $"adaptiveStickSaturated={adaptiveStickSaturated} " +
            $"naturalMotionReleased={naturalMotionReleased}", this);
    }
