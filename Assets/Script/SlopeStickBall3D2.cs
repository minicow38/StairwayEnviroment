...
public class SlopeStickBall3D2 : MonoBehaviour
{
    ...
    [Header("Motion Snapshot")]
    [Tooltip("初回基準の採取と、面切替・Soft Handoff・ステージ回転で共通使用するSamplerです。")]
    [SerializeField] ReliableBaselineSampler reliableBaselineSampler;

    [Tooltip("ONなら、面切替・Soft Handoff・ステージ回転を一つのSnapshot機構で処理します。")]
    [SerializeField] bool useMotionSnapshot = true;

    [Tooltip("前回の接地法線からこの角度以上変わった時に、面切替Snapshotを開始します。")]
    [SerializeField, Range(.1f, 89f)] float motionSnapshotNormalChangeTriggerAngle = 5f;

    [Tooltip("現在法線と目標法線の差がこの角度以内なら、面が安定した候補と判断します。")]
    [SerializeField, Range(.1f, 15f)] float motionSnapshotStableNormalTolerance = 2f;

    [Tooltip("安定候補がこのFixedUpdate数だけ続いた後、保存した前進速度と転がり角速度を再構成します。")]
    [SerializeField, Range(1, 10)] int motionSnapshotStableFrames = 3;

    [Tooltip("保存直後の同一FixedUpdateで復元しないための最低保持時間です。")]
    [SerializeField, Min(0f)] float motionSnapshotMinimumSeconds = .06f;
    ...
    MotionSnapshotState motionSnapshot;
    ...
    void OnValidate()
    {
        ...
        motionSnapshotNormalChangeTriggerAngle = Mathf.Clamp(motionSnapshotNormalChangeTriggerAngle, .1f, 89f);
        motionSnapshotStableNormalTolerance = Mathf.Clamp(motionSnapshotStableNormalTolerance, .1f, 15f);
        motionSnapshotStableFrames = Mathf.Clamp(motionSnapshotStableFrames, 1, 10);
        motionSnapshotMinimumSeconds = Mathf.Max(0f, motionSnapshotMinimumSeconds);
    ...
    void FixedUpdate()
    {
        ...
        bool restoredSnapshot = RestorePendingMotionSnapshot();
        ...
        UpdateMotionSnapshotCompletion();
    }
    ...
    void RotateHeadingAndStage(Vector3 startDir, float playerAngle)
    {
        ...
        RestoreInterruptedMotionSnapshot();
        headingTween?.Kill();
        ...
        BeginMotionSnapshotForStageTurn(startDir);
        ...
        if (duration <= 0f || Mathf.Abs(playerAngle) <= .001f)
        {
            ...
            CompleteMotionSnapshotAfterStageTurn(finalStageTurn, targetHeadingDir);
            return;
        }
    ...
    void DisableBallControllerDuringInitialBaseline()
    {
        ...
        // Rigidbody本体には触れず、コントローラー内部状態だけを初期化する。
        flatToSlopeApproachState = default;
    ...
    void DetectAbruptSurfaceNormalChange(Vector3 detectedNormal)
    {
        ...
        // ステージ回転中の法線変化は、同じMotion Snapshotの回転イベント側で処理する。
        if (stageTurnInProgress || motionSnapshot.pendingRestore)
        {
            previousSurfaceNormal = detectedNormal;
            return;
        }
        ...
        if (useMotionSnapshot && normalChange >= motionSnapshotNormalChangeTriggerAngle)
        {
            Vector3 newHeading = ProjectHeadingOnSurface(headingDir, detectedNormal);
            ArmMotionSnapshotForSurface(
                previousSurfaceNormal,
                detectedNormal,
                newHeading,
                MotionSnapshotReason.AbruptSurfaceNormalChange);
        }

        previousSurfaceNormal = detectedNormal;
    }

    void ArmMotionSnapshotForSurface(
        Vector3 sourceNormal,
        Vector3 targetNormal,
        Vector3 targetHeading,
        MotionSnapshotReason reason)
    {
        if (stageTurnInProgress)
            return;

        TryArmMotionSnapshot(
            sourceNormal,
            targetNormal,
            targetHeading,
            reason);
    ...
    bool TryArmMotionSnapshot(
        Vector3 sourceNormal,
        Vector3 targetNormal,
        Vector3 targetHeading,
        MotionSnapshotReason reason)
    {
        if (!useMotionSnapshot || !rb || !HasInitialReliableBaselineSnapshot)
            return false;
        ...
        switch (reason)
        {
            case MotionSnapshotReason.BeforeSoftHandoffVelocityChange:
                pieceType = ReliableBaselineSampler.CyclePieceType.BeforeVelocityChange;
                break;

            case MotionSnapshotReason.BeforeDOTweenTurn:
                pieceType = ReliableBaselineSampler.CyclePieceType.BeforeDOTweenTurn;
                break;

            default:
                pieceType = ReliableBaselineSampler.CyclePieceType.AbruptNormalChange;
                break;
        }
        ...
        if (!reliableBaselineSampler.TryGetTrustedPiece(
                pieceType,
                out ReliableBaselineSampler.BaselineResult trustedResult))
        {
            return false;
        }
        ...
        motionSnapshot = new MotionSnapshotState
        {
            valid = true,
        ...
        {
            Debug.Log(
                $"[Motion Snapshot Armed] reason={reason} " +
                $"trustedCycles={reliableBaselineSampler.TrustedCycleCount} " +
                $"time={motion.savedTime:F4} forward={motion.forwardSpeed:F4} " +
                $"rolling={motion.rollingAngularSpeed:F4}",
                this);
        }
    ...
    void UpdateMotionSnapshotCompletion()
    {
        if (!motionSnapshot.valid || motionSnapshot.pendingRestore ||
            motionSnapshot.reason == MotionSnapshotReason.BeforeDOTweenTurn)
        {
            return;
        }
        ...
        if (!isGrounded)
        {
            motionSnapshot.stableFrames = 0;
            return;
        }

        if (Time.fixedTime - motionSnapshot.startedTime < motionSnapshotMinimumSeconds)
            return;

        // Soft Handoffの短いReleaseが終わってから、一度だけ保存状態へ戻す。
        ...
        float normalError = Vector3.Angle(groundNormal, motionSnapshot.targetNormal);

        if (normalError <= motionSnapshotStableNormalTolerance)
            motionSnapshot.stableFrames++;
        else
            motionSnapshot.stableFrames = 0;

        if (motionSnapshot.stableFrames < motionSnapshotStableFrames)
            return;

        ReconstructMotionFromSnapshot(
            motionSnapshot.motion,
            groundNormal,
            motionSnapshot.targetHeading,
            motionSnapshot.reason.ToString());

        motionSnapshot = default;
    }

    void BeginMotionSnapshotForStageTurn(Vector3 startHeading)
    {
        stageTurnInProgress = true;

        if (!HasInitialReliableBaselineSnapshot || !useMotionSnapshot || !rb)
        {
            motionSnapshot = default;
            return;
        }
        ...
        TryArmMotionSnapshot(
            ...
            sourceNormal,
            sourceHeading,
            MotionSnapshotReason.BeforeDOTweenTurn);
    }

    void CompleteMotionSnapshotAfterStageTurn(Quaternion finalStageTurn, Vector3 finalHeading)
    {
        stageTurnInProgress = false;

        if (!motionSnapshot.valid || motionSnapshot.reason != MotionSnapshotReason.BeforeDOTweenTurn)
            return;

        Vector3 startNormal = motionSnapshot.targetNormal;
        motionSnapshot.targetNormal = NormalizeOrFallback(finalStageTurn * startNormal, startNormal);
        motionSnapshot.targetHeading = ProjectHeadingOnSurface(finalHeading, motionSnapshot.targetNormal);
        motionSnapshot.pendingRestore = true;

        if (debugMotionSnapshot)
        {
            Debug.Log(
                $"[Motion Snapshot Awaiting Restore] reason=BeforeDOTweenTurn " +
                $"targetNormal={motionSnapshot.targetNormal:F4} " +
                $"targetHeading={motionSnapshot.targetHeading:F4}",
                this);
        }
    }

    bool RestorePendingMotionSnapshot()
    {
        if (!motionSnapshot.valid || !motionSnapshot.pendingRestore)
            return false;

        Vector3 targetNormal = isGrounded
            ? NormalizeOrFallback(groundNormal, motionSnapshot.targetNormal)
            : NormalizeOrFallback(motionSnapshot.targetNormal, Vector3.up);

        Vector3 targetHeading = ProjectHeadingOnSurface(motionSnapshot.targetHeading, targetNormal);
    ...
    void RestoreInterruptedMotionSnapshot()
    {
        if (!motionSnapshot.valid)
            return;

        Vector3 targetNormal = NormalizeOrFallback(groundNormal, motionSnapshot.motion.sourceNormal);
        ...
        ReconstructMotionFromSnapshot(
            ...
            targetNormal,
            targetHeading,
            $"Interrupted/{motionSnapshot.reason}");

        motionSnapshot = default;
        stageTurnInProgress = false;
    }

    void ReconstructMotionFromSnapshot(
        MotionSnapshotData motion,
        Vector3 targetNormal,
        Vector3 targetHeading,
        string reason)
    {
        ...
        BuildSurfaceBasis(
            targetNormal,
            targetHeading,
            out Vector3 rebuiltNormal,
            out Vector3 rebuiltHeading,
            out Vector3 targetSide);

        targetNormal = rebuiltNormal;
        targetHeading = rebuiltHeading;

        // 必要最低限だけ戻す。
        // 前進速度は新しい進行方向へ移し、横速度は復元しない。
        // 法線速度は現在の面向き成分だけを残し、外向き成分は復元しない。
        float inwardNormalSpeed = Mathf.Min(Vector3.Dot(rb.velocity, targetNormal), 0f);
        Vector3 targetVelocity =
            targetHeading * motion.forwardSpeed +
            targetNormal * inwardNormalSpeed;

        ApplyVelocitySnapshotCorrection(targetVelocity);

        // 転がり成分だけを新しい横軸へ移す。
        // 縦回転・ヨー回転・進行軸回転は復元しない。
        rb.angularVelocity = targetSide * motion.rollingAngularSpeed;
    ...
    static void BuildSurfaceBasis(
        Vector3 normalInput,
        Vector3 headingInput,
        out Vector3 normal,
        out Vector3 heading,
        out Vector3 side)
    {
    ...
    void LogMotionSnapshotReconstruction(
        string reason,
        MotionSnapshotData motion,
        Vector3 targetNormal,
        Vector3 targetHeading)
    {
        ...
        Debug.Log(
            $"[Motion Snapshot Reconstructed] reason={reason} time={Time.fixedTime:F4} " +
            $"savedVelocity={motion.velocity:F4} savedAngularVelocity={motion.angularVelocity:F4} " +
            $"targetNormal={targetNormal:F4} targetHeading={targetHeading:F4} " +
            $"velocity={rb.velocity:F4} angularVelocity={rb.angularVelocity:F4}",
            this);
    }
    ...
    {
        if (Physics.SphereCast(transform.position + Vector3.up * .02f, sphereRadius * .95f, Vector3.down, out hit, sphereRadius + groundProbeDistance,
                groundMask, QueryTriggerInteraction.Ignore) && Vector3.Angle(hit.normal, Vector3.up) <= maxSlopeAngle)
        {
            return true;
        }
    ...
    void ApplySoftSlopeHandoffVelocityChange()
    {
        ...
        ArmMotionSnapshotForSurface(flatNormal, slopeNormal, slopeTangent, MotionSnapshotReason.BeforeSoftHandoffVelocityChange);
