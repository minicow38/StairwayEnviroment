...
public sealed class SlopeStick3DCompact2 : MonoBehaviour
{
    const string ImplementationVersion = "CompactInvariantController-v2.1-MathUnified-2026-07-29";
    const float Epsilon = 0.000001f;
    static readonly Dictionary<int, SlopeStick3DCompact2> Owners = new Dictionary<int, SlopeStick3DCompact2>();
    enum GroundKind
    {
    ...
    {
        None, SphereCast, SlopeEntryBridge, SurfaceObserverGrace
    }
    enum TargetPhase
    {
    ...
    void Start() {
        ...
        if (logEvents) {
            ...
            Debug.Log(
                $"[COMPACT DETECTION CONFIG] " +
                $"sphereRadius={sphereRadius:F4} " +
                $"groundProbeDistance={groundProbeDistance:F4} " +
                $"bridgeEndProgress={bridgeEndProgress:F4} " +
                $"bridgeMaximumGap={bridgeMaximumGap:F4} " +
                $"bridgeMaximumOutwardSpeed={bridgeMaximumOutwardSpeed:F4} " +
                $"forwardSlopeProbeDistance={forwardSlopeProbeDistance:F4} " +
                $"forwardSlopeProbeSegments={forwardSlopeProbeSegments} " +
                $"forwardProbeHeight={forwardProbeHeight:F4} " +
                $"forwardProbeDownDistance={forwardProbeDownDistance:F4}",
                this);
        }
    ...
    void AcquireOwnership() {
        ...
        if (Owners.TryGetValue(key, out SlopeStick3DCompact2 owner) && owner != null && owner != this) {
    ...
    void ReleaseOwnership() {
        if (!ownsRigidbody || rb == null)
            return;
        int key = rb.GetInstanceID();
        if (Owners.TryGetValue(key, out SlopeStick3DCompact2 owner) && owner == this) {
            Owners.Remove(key);
        }
        ownsRigidbody = false;
    }
