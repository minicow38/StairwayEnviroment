...
public class BallVisualSlopeDrive1 : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        if (isOnSlope)
        {
            ...
            {
                Debug.Log("");
                //if(rb.velocity.sqrMagnitude<decisiveValue)
                /*rb.velocity = respondSubject.mappedvelocity;
                rb.angularVelocity = respondSubject.mappedAngularVelocity;*/
               
                //rb.position = respondSubject.mappedPosition;
               // rb.rotation = mappedRotation;
                Debug.Log("");
            }
            /*rb.position = respondSubject.MappedPosition;
    ...
    private void ProcessSlope()
{
    ...
    private void ProcessRejoin()
    {
        ...
        // 臨界減衰型の目標加速度。
        // 最初は速度差を優先し、位置項はSmoothStepで徐々に立ち上げる。
        float omega =
            2f /
