...
public sealed class SlopeStick3D4 : MonoBehaviour
{
    ...
    bool TryBuildSlopeEntrySupportBridge(int nextMissFrames, float dt, out GroundObservation observation)
    {
        ...
        if (!useLocalFlatToSlopeSequencePatch || nextMissFrames != LocalSlopeEntryBridgeMaximumMissFrames || !wasGrounded ||
            ...
            !slopeFrame.valid || !slopeFrame.collider || !trackedSlopeCollider || trackedSlopeCollider != slopeFrame.collider ||
            !lastMeasuredSlopeObservation.valid || lastMeasuredSlopeObservation.collider != slopeFrame.collider)
        {
            return false;
        }
    ...
    {
        if (!observation.valid || !observation.collider || !slopeFrame.valid || observation.collider != slopeFrame.collider)
            return;
    ...
    void ApplySlopeEntryVelocityTransportOnce()
    {
        if (!useSlopeEntryVelocityTransport ||
            ...
            !slopeFrame.valid ||
            !slopeFrame.collider ||
            slopeEntryVelocityTransportCollider == slopeFrame.collider ||
            anticipatedSlopeEntryCollider != slopeFrame.collider)
        {
            return;
        }
    ...
    bool BuildSlopeFrame(Collider slopeCollider, Vector3 referenceNormalInput, Vector3 desiredForward, out SlopeFrame frame)
    {
        ...
        // slopeProgressStatePercent は、この1枚の斜面Colliderの入口0%～出口100%を表します。
        ...
        float projectedHalfExtent = Mathf.Abs(axis.x) * extents.x + Mathf.Abs(axis.y) * extents.y + Mathf.Abs(axis.z) * extents.z;
        float minimum = centerCoordinate - projectedHalfExtent;
        float maximum = centerCoordinate + projectedHalfExtent;
        ...
        float releaseFactor, float dt)
    {
        ...
        float requiredStick = CalculateRequiredBaseStickForSpeed(speedForecast, sample.curvature, sample.gravitySupport);
        ...
        if (adaptiveStickSaturated)
        {
            if (!adaptiveStickSaturationLogged && writeRuntimeLog && logAdaptiveCriticalBaseStick)
            {
                ...
                Debug.LogWarning($"[ADAPTIVE CRITICAL STICK SATURATED] time={Time.fixedTime:F4}s " + $"collider={frame.collider.name} progress={progress * 100f:F3}% " +
                    $"speed={currentSpeed:F6} speedForecast={speedForecast:F6} " + $"required={requiredStick:F6} damping={outwardDamping:F6} " +
                    $"stickLimit={stickLimit:F6} maxGroundSpeed={maxGroundSpeed:F6} " + $"maximumAdaptive={maximumAdaptiveBaseStickAcceleration:F6}", this);
            }
        }
        else
        {
        ...
        float distanceToSlopeEntry, SurfaceBasis basis, float dt)
    {
        ...
        if (logLocalFlatToSlopeSequencePatch && writeRuntimeLog &&
            (!slopeEntryBrakeLogged || slopeEntryBrakeLoggedCollider != evaluationFrame.collider))
        {
        ...
        SurfaceSamplePurpose purpose)
    {
        ...
        for (int hitIndex = 0; hitIndex < hits.Length; hitIndex++)
        {
            ...
            // SlopeFrameは現在の1枚だけを表し、親Root配下の別Colliderは承認しません。
            if (!frame.valid || !hit.collider || frame.collider != hit.collider)
                continue;
